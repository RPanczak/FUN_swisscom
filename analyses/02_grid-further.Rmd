...
The do have unique ID so can easily be excluded in order to create correct visualizations (see [#8](https://github.com/RPanczak/FUN_swisscom/issues/8)). However analyses that would be based on PLZs, particularly aggregation of data would have to determine correct assignment of grid cells to PLZs. Perhaps by using (pop weighted?) centroid or sth similar?  
